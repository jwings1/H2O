...
for lr, bs, dr, layers in itertools.product(
    ...
    class BehaveDataset(Dataset):
        def __init__(
            self,
            reprojected_smpl_cam0_tensors,
            reprojected_smpl_cam1_tensors,
            reprojected_smpl_cam2_tensors,
            reprojected_smpl_cam3_tensors,
            reprojected_obj_cam0_tensors,
            reprojected_obj_cam1_tensors,
            reprojected_obj_cam2_tensors,
            reprojected_obj_cam3_tensors,
            identifiers,
            num_mlps,
        ):
            self.reprojected_smpl_cam0 = reprojected_smpl_cam0_tensors
            self.reprojected_smpl_cam1 = reprojected_smpl_cam1_tensors
            self.reprojected_smpl_cam2 = reprojected_smpl_cam2_tensors
            self.reprojected_smpl_cam3 = reprojected_smpl_cam3_tensors

            self.reprojected_obj_cam0 = reprojected_obj_cam0_tensors
            self.reprojected_obj_cam1 = reprojected_obj_cam1_tensors
            self.reprojected_obj_cam2 = reprojected_obj_cam2_tensors
            self.reprojected_obj_cam3 = reprojected_obj_cam3_tensors
    ...
    ############## USING A SUBSET ######################
    all_files = sorted(glob.glob(os.path.join(base_path, "sequences", "Date07_Sub04_yogaball_play", "t*.000")))[:10]
    ...
    def sliding_window_grouping(input_list, N_MLP):
        """
        ...
        for i in range(len(input_list) - N_MLP + 1):
            window = input_list[i : i + N_MLP]
            tensor = torch.tensor(window, dtype=torch.float32)
            tensors.append(tensor)
        return tensors

    reprojected_smpl_cam1_tensors = sliding_window_grouping(reprojected_smpl_cam1_list, N_MLPS)
    reprojected_smpl_cam0_tensors = sliding_window_grouping(reprojected_smpl_cam0_list, N_MLPS)
    reprojected_smpl_cam2_tensors = sliding_window_grouping(reprojected_smpl_cam2_list, N_MLPS)
    reprojected_smpl_cam3_tensors = sliding_window_grouping(reprojected_smpl_cam3_list, N_MLPS)
    reprojected_obj_cam1_tensors = sliding_window_grouping(reprojected_obj_cam1_list, N_MLPS)
    reprojected_obj_cam0_tensors = sliding_window_grouping(reprojected_obj_cam0_list, N_MLPS)
    reprojected_obj_cam2_tensors = sliding_window_grouping(reprojected_obj_cam2_list, N_MLPS)
    reprojected_obj_cam3_tensors = sliding_window_grouping(reprojected_obj_cam3_list, N_MLPS)
    # identifiers_tensors = sliding_window_grouping(identifiers, N_MLPS)

    input_dim = 75  # reprojected_smpl_cam0_list[0].shape[0]
    output_dim = 24  # reprojected_obj_cam0_list[0].shape[0]

    # print(reprojected_smpl_cam1_tensors[0].shape)
