...
for lr, bs, dr, layers in itertools.product(
    ...
    src_data = [
        torch.cat(
            [
                reprojected_smpl_cam1_dist[i],
                reprojected_smpl_cam0_dist[i],
                reprojected_smpl_cam2_dist[i],
                reprojected_smpl_cam3_dist[i],
            ]
        )
        for i in range(len(reprojected_smpl_cam1_dist))
    ]

    tgt_data = [
        torch.cat(
            [
                reprojected_smpl_cam1_tri[i],
                reprojected_smpl_cam0_tri[i],
                reprojected_smpl_cam2_tri[i],
                reprojected_smpl_cam3_tri[i],
            ]
        )
        for i in range(len(reprojected_smpl_cam1_tri))
    ]
