...
for lr, bs, dr, layers_1, layers_3, alpha, lambda_1, lambda_2, l, epochs, optimizer_name in itertools.product(
    ...
    class CombinedMLP(pl.LightningModule):
        ...
        def forward(self, x, joints, obj):
            ...
            def process_pose_params_batch(batch_pose_params):
                # Ensure batch_pose_params is a NumPy array
                batch_pose_params = np.array(batch_pose_params)
                ...
                for b in range(batch_size):
                    pose_params = batch_pose_params[b].reshape(-1, 3)
                    absolute_rotations = process_pose_params(pose_params)
                    batch_results.append(absolute_rotations)

                # Reshape the results to (batch_size, 72)
                return torch.tensor(batch_results, dtype=torch.float32).view(batch_size, -1)
        ...
        def training_step(self, batch, batch_idx):
            ...
            loss_cam1_stage2 = (
                ...
                + lambda_1 * F.cosine_similarity(y_hat_cam1_stage2, y_cam1_stage2)
            ...
            loss_cam0_stage2 = (
                ...
                + lambda_1 * F.cosine_similarity(y_hat_cam0_stage2, y_cam0_stage2)
            ...
            loss_cam3_stage2 = (
                ...
                + lambda_1 * F.cosine_similarity(y_hat_cam3_stage2, y_cam3_stage2)
            ...
            loss_cam2_stage2 = (
                ...
                + lambda_1 * F.cosine_similarity(y_hat_cam2_stage2, y_cam2_stage2)
                + lambda_2 * smooth_sign_loss(y_hat_cam2_stage2, y_cam2_stage2)
            )

            # Ignore cam 1
            total_loss = (0 * loss_cam1_stage2 + loss_cam0_stage2 + loss_cam2_stage2 + loss_cam3_stage2) ** 2
        ...
        def configure_optimizers(self):
            ...
            scheduler = {
                "scheduler": CustomCyclicLR(optimizer, base_lr=5e-6, max_lr=5e-4, step_size=1, mode="exp_range"),
    ...
    model_path = f"/scratch_net/biwidl307/lgermano/H2O/trained_models/pos_enc/model_decent-eon-2053_epoch_46.pt"
    ...
    trainer = pl.Trainer(max_epochs=wandb.config.epochs, num_sanity_val_steps=0)
