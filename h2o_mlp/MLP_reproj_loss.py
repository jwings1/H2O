...
for lr, bs, dr, layers in itertools.product(
    ...
    class MLP(pl.LightningModule):
        ...
        def training_step(self, batch, batch_idx):
            ...
            cam_params = load_config(1, base_path)
            candidate_obj_meshes_1 = transform_obj_mesh_to_camera_frame(
                y_cam1_hat[:, :3], y_cam1_hat[:, -3:], cam_params, object_mesh
            )
            ...
            candidate_obj_meshes_0 = transform_obj_mesh_to_camera_frame(
                y_cam0_hat[:, :3], y_cam0_hat[:, -3:], cam_params, object_mesh
            )
            ...
            cam_params = load_config(1, base_path)  # Adjusted the camera index
            candidate_obj_meshes_2 = transform_obj_mesh_to_camera_frame(
                y_cam2_hat[:, :3], y_cam2_hat[:, -3:], cam_params, object_mesh
            )
            ...
            cam_params = load_config(1, base_path)  # Adjusted the camera index
            candidate_obj_meshes_3 = transform_obj_mesh_to_camera_frame(
                y_cam3_hat[:, :3], y_cam3_hat[:, -3:], cam_params, object_mesh
            )
            ...
            def vertex_to_vertex_loss(predicted_meshes, gt_meshes):
                ...
                """
                squared_errors = [
                    (torch.tensor(pred) - torch.tensor(gt)) ** 2 for pred, gt in zip(predicted_meshes, gt_meshes)
                ]
            ...
            candidate_mesh_lists = [
                candidate_obj_mesh_0_np_list,
                candidate_obj_mesh_1_np_list,
                candidate_obj_mesh_2_np_list,
                candidate_obj_mesh_3_np_list,
            ]
            losses = [
                vertex_to_vertex_loss(candidates, gt_obj_mesh_vertices_np_list) for candidates in candidate_mesh_lists
            ]
