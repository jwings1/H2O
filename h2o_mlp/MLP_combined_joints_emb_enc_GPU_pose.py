...
epochs_range = [3000]  # ,300]
learning_rate_range = [1e-4]  # , 5e-3, 1e-2, 5e-2]
batch_size_range = [32]  # [32, 64, 128]
...
optimizer_list = ["Adam"]  # ["SGD", "Adagrad", "RMSprop", "AdamW", "Adadelta", "LBFGS"]
...
for lr, bs, dr, layers_1, layers_3, alpha, lambda_1, lambda_2, l, epochs, optimizer_name in itertools.product(
    ...
    def load_frames_trilateration(interpolated_data_frames):
        ...
        for idx, frame_data in enumerate(interpolated_data_frames):
            ...
            for cam_id in [1, 0, 2, 3]:
                ...
                camera1_params = load_config(1, base_path, "Date01")
                cam_params = load_config(cam_id, base_path, "Date01")
                transformed_smpl = transform_smpl_to_camera_frame(pose, trans, camera1_params, cam_params)

                if cam_id == 1:
                    reprojected_obj_cam1_list.append(obj_pose)
                ...
                if cam_id == 0:
                    ...
                    data["angle"] = obj_pose
                    data["trans"] = obj_trans
                    obj_pose = transform_object_to_camera_frame(data, camera1_params, cam_params)
    ...
    class BehaveDataset(Dataset):
        def __init__(
            self,
            smpl_inputs_dist,
            smpl_reprojected_cam0_dist,
            smpl_reprojected_cam2_dist,
            smpl_reprojected_cam3_dist,
            obj_labels_dist,
            obj_reprojected_cam0_dist,
            obj_reprojected_cam2_dist,
            obj_reprojected_cam3_dist,
            smpl_inputs_tri,
            smpl_reprojected_cam0_tri,
            smpl_reprojected_cam2_tri,
            smpl_reprojected_cam3_tri,
            obj_labels_tri,
            obj_reprojected_cam0_tri,
            obj_reprojected_cam2_tri,
            obj_reprojected_cam3_tri,
            identifiers,
        ):
    ...
    class CombinedMLP(pl.LightningModule):
        ...
        def forward(self, x, joints, obj):
            ...
            def axis_angle_to_rotation_matrix(axis_angle):
                ...
                cross_prod_matrix = torch.tensor(
                    [[0, -axis[2], axis[1]], [axis[2], 0, -axis[0]], [-axis[1], axis[0], 0]]
                ).to(device)
                return (
                    torch.eye(3).to(device) * cos_theta
                    + (1 - cos_theta) * torch.ger(axis, axis)
            ...
            def process_pose_params(pose_params):
                ...
                parents = torch.tensor(
                    [-1, 0, 0, 0, 1, 2, 3, 4, 5, 3, 7, 8, 9, 9, 9, 12, 13, 14, 16, 17, 18, 19, 20, 21]
                ).to(device)
                ...
                absolute_rotations_axis_angle = [
                    rotation_matrix_to_axis_angle(rot) for rot in absolute_rotations_matrices
                ]
            ...
            def process_pose_params_batch(batch_pose_params):
                ...
                for b in range(batch_size):
                    ...
                    batch_results.append(
                        torch.stack(absolute_rotations)
                    )  # Assuming absolute_rotations is a list of tensors
        ...
        def training_step(self, batch, batch_idx):
            ...
            # Loss for camera 1
            loss_cam1_stage2 = (
                lambda_1 * F.mse_loss(y_hat_cam1_stage2, y_cam1_stage2)
                + lambda_1 * (1 - F.cosine_similarity(y_hat_cam1_stage2, y_cam1_stage2))
                + lambda_2 * smooth_sign_loss(y_hat_cam1_stage2, y_cam1_stage2)
            )
            ...
            # Loss for camera 0
            loss_cam0_stage2 = (
                lambda_1 * F.mse_loss(y_hat_cam0_stage2, y_cam0_stage2)
                + lambda_1 * (1 - F.cosine_similarity(y_hat_cam0_stage2, y_cam0_stage2))
                + lambda_2 * smooth_sign_loss(y_hat_cam0_stage2, y_cam0_stage2)
            )
            ...
            # Loss for camera 3
            loss_cam3_stage2 = (
                lambda_1 * F.mse_loss(y_hat_cam3_stage2, y_cam3_stage2)
                + lambda_1 * (1 - F.cosine_similarity(y_hat_cam3_stage2, y_cam3_stage2))
                + lambda_2 * smooth_sign_loss(y_hat_cam3_stage2, y_cam3_stage2)
            )
            ...
            # Loss for camera 2
            loss_cam2_stage2 = (
                lambda_1 * F.mse_loss(y_hat_cam2_stage2, y_cam2_stage2)
                + lambda_1 * (1 - F.cosine_similarity(y_hat_cam2_stage2, y_cam2_stage2))
                + lambda_2 * smooth_sign_loss(y_hat_cam2_stage2, y_cam2_stage2)
            )
        ...
        def configure_optimizers(self):
            ...
            scheduler = {
                "scheduler": CustomCyclicLR(optimizer, base_lr=1e-7, max_lr=5e-5, step_size=1, mode="exp_range"),
    ...
    data_file_path = (
        "/scratch_net/biwidl307/lgermano/H2O/datasets/MLP_combined/Date01_Sub01_backpack_back_dataset_poses_3frames.pkl"
    )
    ...
    if os.path.exists(data_file_path):
        ...
        (
            ...
            reprojected_obj_cam3_dist,
            identifiers_dist,
        ) = load_frames_distance_regressor(interpolated_data_frames)
        ...
        (
            ...
            reprojected_obj_cam3_tri,
            identifiers_tri,
        ) = load_frames_trilateration(interpolated_data_frames)
    ...
    model_path = f"/scratch_net/biwidl307/lgermano/H2O/trained_models/pos_enc/model_azure-blaze-2150_epoch_10.pt"
