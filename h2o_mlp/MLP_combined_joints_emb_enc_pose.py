...
epochs_range = [15, 25]  # ,300]
# learning_rate_range = [1e-3, 5e-3, 1e-2, 5e-2]
batch_size_range = [16, 32, 64, 128]
dropout_rate_range = [0]
alpha_range = [10, 100, 1000]  # [0.01,0.1,1,10,100]
lambda_1_range = [10, 100, 1000]  # [0.01,0.1,1,10,100]
lambda_2_range = [0.01, 1, 10]  # [0.0001,0.001,0.01,0.1,1]
L_range = [4, 6, 10]
layer_sizes_range_1 = [
    # [128, 128, 64, 64, 32],
    # [256, 256, 128, 128, 64],
...
for lr, bs, dr, layers_1, layers_3, alpha, lambda_1, lambda_2, l, epochs in itertools.product(
    learning_rate_range,
    batch_size_range,
    dropout_rate_range,
    layer_sizes_range_1,
    ...
    class CombinedMLP(pl.LightningModule):
        ...
        def training_step(self, batch, batch_idx):
            ...
            # Original stage2
            loss_cam1_stage2 = lambda_1 * F.mse_loss(y_hat_cam1_stage2, y_cam1_stage2) + lambda_2 * smooth_sign_loss(
                y_hat_cam1_stage2, y_cam1_stage2
            )
            loss_cam0_stage2 = lambda_1 * F.mse_loss(y_hat_cam0_stage2, y_cam0_stage2) + lambda_2 * smooth_sign_loss(
                y_hat_cam0_stage2, y_cam0_stage2
            )
            loss_cam3_stage2 = lambda_1 * F.mse_loss(y_hat_cam3_stage2, y_cam3_stage2) + lambda_2 * smooth_sign_loss(
                y_hat_cam3_stage2, y_cam3_stage2
            )
            loss_cam2_stage2 = lambda_1 * F.mse_loss(y_hat_cam2_stage2, y_cam2_stage2) + lambda_2 * smooth_sign_loss(
                y_hat_cam2_stage2, y_cam2_stage2
            )
        ...
        def test_step(self, batch, batch_idx):
            ...
            wandb.log({"loss_test_stage1": loss_original_stage1.item(), "loss_test_stage2": loss_cam1_stage2.item()})
            return avg_loss
    ...
    data_file_path = (
        "/scratch_net/biwidl307/lgermano/H2O/datasets/MLP_combined/Date07_Sub04_yogaball_play_dataset_6frames.pkl"
    )
