...
lambda_1_range = [0.01, 0.1, 1, 10, 100]
lambda_2_range = [0.0001, 0.001, 0.01, 0.1, 1]
layer_sizes_range = [
    # [128, 128, 64, 64, 32],
    # [256, 256, 128, 128, 64],
    [512, 512, 256, 256, 128],
...
for lr, bs, dr, layers, alpha, lambda_1, lambda_2, epochs in itertools.product(
    ...
    def interpolate_frames(all_vectors):
        interpolated_frames = []

        for idx in range(len(all_vectors) - 1):
            frame1 = all_vectors[idx]
            frame2 = all_vectors[idx + 1]
        ...
        interpolated_frames.append(all_vectors[-1])
    ...
    def load_frames_distance_regressor(interpolated_vectors):
        ...
        for idx, frame_data in enumerate(interpolated_vectors):
    ...
    def load_frames_trilateration(interpolated_vectors):
        reprojected_smpl_cam1_list = []
        reprojected_smpl_cam0_list = []
        reprojected_smpl_cam2_list = []
        reprojected_smpl_cam3_list = []
    ...
    if os.path.exists(data_file_path):
        ...
        for idx, frame_folder in enumerate(selected_files):
            ...
            all_vectors.append(frame_data)

        # Interpolate between frames

        interpolated_vectors = interpolate_frames(all_vectors)
        ...
        (
            ...
            reprojected_obj_cam3_dist,
            identifiers_dist,
        ) = load_frames_distance_regressor(interpolated_vectors)
        ...
        (
            ...
            reprojected_obj_cam3_tri,
            identifiers_tri,
        ) = load_frames_trilateration(interpolated_vectors)
    ...
    trainer.fit(combined_model, datamodule=data_module)
