...
for lr, bs, dr, layers in itertools.product(
            ...
            for cam_id in [1, 0, 2, 3]:
                ...
                if cam_id == 1:
                    ...
                    input_tensor = torch.tensor(
                        np.concatenate([transformed_smpl[:72], transformed_smpl[-3:]]), dtype=torch.float32
                    )
    ...
    class CombinedMLP(pl.LightningModule):
        def __init__(self, input_dim, output_stage1, input_stage2, output_dim):
            ...
            self.model2 = MLP_trilateration(input_stage2, output_dim)
            self.automatic_optimization = False
        ...
        def forward(self, x, joints):
            ...
            concatenated_input = torch.cat((x_stage1, joints[:, :72]), dim=1)
        ...
        def training_step(self, batch, batch_idx):
            ...
            total_loss1 = (loss_original_stage1 + loss_cam0_stage1 + loss_cam2_stage1 + loss_cam3_stage1) / 4
            total_loss2 = (loss_original_stage2 + loss_cam0_stage2 + loss_cam2_stage2 + loss_cam3_stage2) / 4

            # Sum the losses
            alpha = 0.7
            avg_loss = alpha * total_loss1 + (1 - alpha) * total_loss2
        ...
        def test_step(self, batch, batch_idx):
            ...
            wandb.log(
                {"loss_test_stage1": loss_original_stage1.item(), "loss_test_stage2": loss_original_stage2.item()}
            )
            return avg_loss
    ...
    filename = f"/scratch_net/biwidl307/lgermano/H2O/trained_models/double_loss/model_offset_trans_smpl_augmented_{wandb.run.name}.pt"
