    ...
    for idx in range(0, len(cam_data[2]) - frames_subclip + 1, 1):
        ...
        for cam_id in [2]:
            ...
            transformed_object = plot_obj_in_camera_frame(
                candidate_obj_pose[0, -masked_frames, :], candidate_obj_trans[0, -masked_frames, :], obj_template_path
            )
            ...
            cv2.putText(
                img, f"MSE pose: {obj_pose_loss:.4f}", bottomRightCornerOfText1, font, fontScale, fontColor, lineType
            )

            cv2.putText(
                img, f"MSE trans: {obj_trans_loss:.4f}", bottomRightCornerOfText2, font, fontScale, fontColor, lineType
            )
