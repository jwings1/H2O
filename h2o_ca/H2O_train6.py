    ...
    for (
        ...
        class BehaveDataset(Dataset):
                        ...
                        if os.path.exists(file_path):
                            ...
                            with open(file_path, "rb") as f:
                                ...
                                for start_idx in range(
                                    0, len(dataset[cam_id]) - self.frames_subclip, self.frames_subclip
                                ):
                                    # for start_idx in range(len(self.dataset[cam_id]) - 2 * self.frames_subclip, len(self.dataset[cam_id]) - self.frames_subclip, self.frames_subclip):
                                    end_idx = start_idx + self.frames_subclip
            ...
            def __getitem__(self, idx):
                file_path, cam_id, start_idx, end_idx, window_start, window_end = self.data_info[idx]
        ...
        class CombinedTrans(pl.LightningModule):
            ...
            def training_step(self, cam_data):
                ...
                predicted_smpl_pose, predicted_smpl_joints, predicted_obj_pose, predicted_obj_trans = self.forward(
                    cam_data
                )

                # Compute L2 loss (MSE) for both pose and translation
                smpl_pose_loss = F.mse_loss(predicted_smpl_pose, smpl_pose)
                smpl_joints_loss = F.mse_loss(
                    predicted_smpl_joints, smpl_joints.reshape(wandb.config.batch_size, -1, 72)
                )
                ...
                self.log(
                    "train_smpl_pose_loss", smpl_pose_loss, on_step=True, on_epoch=True, prog_bar=True, logger=True
                )
                self.log(
                    "train_smpl_joints_loss", smpl_joints_loss, on_step=True, on_epoch=True, prog_bar=True, logger=True
                )
            ...
            def validation_step(self, cam_data, batch_idx):
                ...
                self.log(
                    "val_smpl_joints_loss", smpl_joints_loss, on_step=True, on_epoch=True, prog_bar=True, logger=True
                )
                self.log("val_trans_loss", trans_loss, on_step=True, on_epoch=True, prog_bar=True, logger=True)
                self.log("val_total_loss", total_loss, on_step=True, on_epoch=True, prog_bar=True, logger=True)
